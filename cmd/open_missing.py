...
def handle_existing_scrcpy(serial, keep=False):
    """Checks for existing process. If keep is True, returns True if running. Else kills it."""
    try:
...
def main():
    ...
    parser.add_argument("--keep", action="store_true", help="Keep existing windows and only open missing ones.")
    ...
    for i, serial in enumerate(serials[:MAX_DEVICES]):
        ...
        if not args.reset:
            is_running = handle_existing_scrcpy(serial, keep=args.keep)
            if is_running:
                print(f"[{serial}] Already running. Skipping.")
                continue
