...
## 🚀 주요 사용법
명령어 구조: `./run.sh <DEVICE_ID 또는 배열_인덱스_번호(0, 1, 2...)> [옵션]`
...
- `--ip` : 스크립트 도중 안드로이드의 비행기 탑승 모드(Airplane Mode)를 강제로 3초간 켰다 끄는 과정을 추가하여, 통신사(LTE/5G)로부터 완전히 새로운 외부 IP를 재발급(순환)받아 접속합니다. 잦은 API 호출로 인해 출발지 IP가 밴(Ban) 당하는 것을 방지합니다.

> 💡 **Tip:** `devices.json`에 할당해야 했던 `mitm_port`는 이제 파일에서 기재할 필요 없이 자동으로 `기본포트(30000) + 인덱스번호 + 1` 로 계산되어 매핑되도록 최적화되었습니다!

### 📝 실행 예시
```bash
# 기기(R5CT20Y2XYE) 초기화 후, 약관동의를 사전 주입하고 목적지(20109650)로 가상 주행 시작
./run.sh R5CT20Y2XYE --reset --agree --id 20109650
```
