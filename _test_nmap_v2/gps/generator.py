...
# Naver Map Route Generation (Optimized for V2)
class NaverCrypto:
    @staticmethod
...
def generate_gps_route(dest_lat, dest_lng, task_id, filename=None):
    LIB_DIR = "/tmp/route_library"
    os.makedirs(LIB_DIR, exist_ok=True)
    if not filename: filename = f"route_{task_id}.json"
    
    # Target radius: 3km to 10km for stability
    min_rad = 3.0; max_rad = 10.0
    best_route_pts = None
    best_dist = float('inf')

    # Convert to float once to avoid repeated conversions
    d_lat = float(dest_lat)
    d_lng = float(dest_lng)

    for attempt in range(1, 6):
        start_lat, start_lng = get_random_point(d_lat, d_lng, min_rad, max_rad)
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("Usage: generator.py <LAT> <LNG> <TASK_ID>")
        sys.exit(1)
    
    lat = sys.argv[1]; lng = sys.argv[2]; tid = sys.argv[3]
    if not generate_gps_route(lat, lng, tid):
        sys.exit(1)
